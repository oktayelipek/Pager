<script>
  ...
  let menuItems = [
    ...
    { value: "productmarketing", label: "Product Marketing", selected: false },
  ...
  function handleChange() {
    ...
    if (selectedTemplate.value === "workinprogress") {
      for (let page of pages.workinprogress) {
        createField(page);
      }
    } else if (selectedTemplate.value === "readyfordev") {
      for (let page of pages.readyfordev) {
        createField(page);
      }
    } else if (selectedTemplate.value === "productmarketing") {
      for (let page of pages.productmarketing) {
        createField(page);
      }
    } else if (selectedTemplate.value === "archive") {
      for (let page of pages.archive) {
        createField(page);
      }
